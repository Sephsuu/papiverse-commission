...
export function BranchPODatePickerModal({
    date,
    mode,
    setDate,
    setEndDate,
    ...
    const [quarterNumber, setQuarterNumber] = useState(Math.floor(baseDate.getMonth() / 3) + 1);
    ...
    }, [mode, open, selectedBranch, selectedDate, today]);
    ...
    const rangeLabel = useMemo(() => {
        ...
        if (periodMode === "MONTH") return format(selectedRange.start, "MMMM yyyy");
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="my-auto max-h-[90vh] overflow-y-auto sm:max-w-xl">
                <DialogTitle>
                    <AppHeader label="Select branch and period" />
                </DialogTitle>
                <div className="space-y-3">
                    <div className="space-y-2 rounded-md border bg-white p-3">
                        <div className="text-sm font-semibold text-slate-700">Branch</div>
                        <AppSelect
                            groupLabel="Branches"
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid gap-2">
                                {quarterOptions.map((quarter) => (
                                    ...
                                    >
                                        <div className={`text-sm font-semibold ${quarterNumber === quarter.value ? "text-white" : "text-slate-900"}`}>{quarter.label}</div>
                                        <div className={`text-xs ${quarterNumber === quarter.value ? "text-white/80" : "text-slate-500"}`}>{quarter.helper}</div>
                                    </button>
                                ))}
                            </div>
                        </div>
