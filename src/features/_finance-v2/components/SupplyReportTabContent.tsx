...
export function SupplyReportTabContent({ startDate, endDate }: SupplyReportTabContentProps) {
	...
	const { search, setSearch, filteredItems } = useSearchFilter(report?.products, ['sku', 'name']);

	const { page, setPage, size, setSize, paginated } = usePagination(filteredItems, 10)
        ...
        <>
            ...
            <div className="mt-4 space-y-4">
				...
				<TableFilter
					search={search}
					setSearch={setSearch}
					searchPlaceholder="Search for supply"
					size={size}
					setSize={setSize}
					removeAdd
					removeFilter
