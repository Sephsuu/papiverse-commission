...
export function SupplyLogsPage() {
    ...
    const { data, loading } = useFetchData<InventoryLog>(
        ...
        [claims.branch.branchId, tab, selectedMonthValue, selectedYear, 'PRODUCT'],
    );
    ...
    useEffect(() => {
        ...
        router.replace(`/logs/supplies?${params.toString()}`);
    }, [tab, eventTab, router]);

    if (authLoading) return <PapiverseLoading />
    return(
        <section className="stack-md animate-fade-in-up overflow-hidden max-md:mt-12">
            <AppHeader label="Supply Inventory Logs" />
