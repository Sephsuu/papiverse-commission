...
import { Supply, supplyInit } from "@/types/supply";
...
export function CreateRawMaterial({ setOpen, setReload, supplies }: Props) {
    const [onProcess, setProcess] = useState(false);
    const [inventorySourceOpen, setInventorySourceOpen] = useState(false);

    const [rawMaterial, setRawMaterial] = useState<Partial<Supply>>(supplyInit);
    ...
    async function handleSubmit() {
        try {         
            ...
            const data = await RawMaterialService.createRawMaterial(rawMaterial);

            if (data) {
                toast.success(`Raw Material ${rawMaterial.name} registered successfully!`); 
                setReload(prev => !prev)
                setOpen(false);
            }   
        }
            ...
            <DialogContent className="max-h-9/10 overflow-y-auto">
                <DialogTitle className="flex items-center gap-2">  
                    <Image
                        ...
                        height={40}
                    />
                    <div className="font-semibold text-xl">Create Raw Material</div>      
                </DialogTitle>
                ...
                >
                    ...
                    <div className="mt-4 flex justify-end gap-4">
                        ...
                        <AddButton 
                            ...
                            loadingLabel="Adding Raw Matrial"
                        />
                    </div>
                </form>
            </DialogContent>
